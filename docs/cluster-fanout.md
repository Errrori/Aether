...
| 突发流量下回读放大（有订阅者的 N 个节点 × 每消息 1 次 SELECT） | 本层接受；候选优化：同频道通知合并为一次 `ReadHistory(afterCursor)` 批量回读（列入第5层候选，见第 8 节） |
...
| 热点频道通知风暴 | 单 goroutine 串行消费，回读延迟上升 | 投递失败 WARN 日志可观测；优化路径见第 8 节 |
...
- **健康与日志**：`/healthz`、`/readyz` 语义不变；降级状态以 WARN 日志（断连/投递失败）观测而非健康检查失败表达。集群指标暂不实现，按需再补。
...
- **Presence 聚合机制复用本通道的可行性**：SPEC 7.6 待细化，依据本通道实测表现决策。
...
可选补充（不阻塞验收）：跨节点端到端延迟实测值、热点频道回读放大测量、并发场景下的乱序率观测。
